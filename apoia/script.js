...
async function renderAlunosPanel(options = {}) {
    ...
    if (defaultToLatestYear && anosLetivosCache.length > 0) {
        currentAnoVal = anosLetivosCache[0];
    }
    anoLetivoFilter.value = currentAnoVal;

    const currentTurmaVal = alunoTurmaFilter.value;
    ...
    alunosTableBody.innerHTML = uniqueStudents.map(aluno => {
        ...
        <tr class="border-b">
            ...
            <td class="p-3 whitespace-nowrap">${aluno.turmas ? aluno.turmas.nome_turma : '<span class="text-orange-500 font-bold">Sem turma</span>'}</td>
            ...
            <td class="p-3 whitespace-nowrap">${actionBtns}</td>
        </tr>`;
    }).join('');
}
...
async function openAlunoModal(editId = null) {
    ...
    if (editId) {
        ...
        if (!data) { showToast('Aluno não encontrado.', true); return; }
...
async function renderApoiaPanel(page = 1) {
    ...
    apoiaTableBody.innerHTML = data.map(item => `
        <tr class="border-b">
            <td class="p-3">${item.alunos ? item.alunos.nome_completo : 'Aluno Removido'}</td>
...
async function openAcompanhamentoModal(editId = null) {
    ...
    if (editId) {
        ...
        if(!data) { showToast('Registro não encontrado.', true); return; }
...
async function renderTurmasPanel() {
    ...
    anoLetivoFilter.value = savedFilter;
...
async function generateAssiduidadeReport() {
    ...
    try {
        ...
        if (activeTab === 'assiduidade-alunos') {
            ...
            if (error) throw error;
            if (!data || data.length === 0) {
                renderReport('<div class="bg-white p-6 rounded-lg shadow-md text-center"><h2 class="font-bold">Nenhum Resultado</h2><p>Nenhum dado encontrado para os filtros selecionados.</p></div>');
                return;
            }
            ...
            if (!dataInicio || !dataFim) {
                const { data: range, error: rangeError } = await db.rpc('get_min_max_presenca_data');
                if (rangeError || !range || !range.min_data) {
                    renderReport('<div class="bg-white p-6 rounded-lg shadow-md text-center"><h2 class="font-bold text-red-600">Erro</h2><p>Não há nenhum registro de chamada no sistema para basear o relatório.</p></div>');
                    return;
                }
                dataInicio = range.min_data;
                dataFim = range.max_data;
            }
        ...
        renderReport(`<div class="bg-white p-6 rounded-lg shadow-md text-center"><h2 class="font-bold text-red-600">Falha na Geração do Relatório</h2></div>`);
    }
}
